...
public class JsonSender implements ISender {
    ...
    public void sendWithTimestamp(String destination, GameObject gameObject, HeaderStatus status) {
        this.template.convertAndSend(destination, gameObject, Map.of("timestamp", System.currentTimeMillis(), "status", status));
    }

    @Override
    public void sendToUser(String destination, GameObject gameObject, HeaderStatus status, String userSession) {
        this.template.convertAndSendToUser(userSession, destination, gameObject, Map.of("timestamp", System.currentTimeMillis(), "status", status));
    }
